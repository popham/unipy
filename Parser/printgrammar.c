...
printnonterminals(grammar *g, FILE *fp)
{
    ...
    for (i = g->g_ndfas; --i >= 0; d++)
        fprintf(fp, "#define %.*s %d\n",
                (int)d->d_name_length, d->d_name, d->d_type);
}
...
printdfas(grammar *g, FILE *fp)
{
    ...
    d = g->g_dfa;
    for (i = 0; i < g->g_ndfas; i++, d++) {
        fprintf(fp, "    {%d, (const unsigned char *)\"%.*s\", %d, %d, %d, states_%d,\n",
                d->d_type,
                (int)d->d_name_length, d->d_name, (int)d->d_name_length,
...
printlabels(grammar *g, FILE *fp)
{
    ...
    for (i = g->g_ll.ll_nlabels; --i >= 0; l++) {
        if (l->lb_str == NULL)
            ...
            fprintf(fp, "    {%d, (const unsigned char *)\"%.*s\"},\n",
                    l->lb_type, (int)l->lb_str_length, l->lb_str);
    }
    fprintf(fp, "};\n");
}
