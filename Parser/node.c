...
PyNode_New(int type)
{
    node *n = PyMem_MALLOC(1 * sizeof(node));
    if (n == NULL)
        return NULL;
    n->n_type = type;
    n->n_str = NULL;
...
int
PyNode_AddChild(register node *n1, int type,
                ...
                int lineno, int col_offset)
{
    ...
    if (current_capacity < required_capacity) {
        ...
        n = n1->n_child;
        n = (node *) PyMem_REALLOC(n, required_capacity * sizeof(node));
        if (n == NULL)
            return E_NOMEM;
        n1->n_child = n;
    }
